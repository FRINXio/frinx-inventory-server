 ...
 * @typedef {Task} Task
 *
 * this Data structure type definition is used only for NexusJS stringified JSONs validation
 *
 * when graphql mutation accepts stringified JSONs as input, we need to validate the JSONs
 *
 * this Type is used only when we need to validate the JSONs that are generated by NexusJS
 * and Conductor API instead of stringified JSONs works with Record<string, unknown> type
 *
 * @readonly
 */
export type Task = {
 ...
 * and Conductor API instead of stringified JSONs works with Record<string, unknown> type
 *
 * this type is used only to define workflowDef property in @typedef {StartWorkflowInput}
 *
 * @readonly
 */
...
/**
 * @typedef {StartWorkflowInput} StartWorkflowInput
 *
 * this Data structure type definition is used only for NexusJS stringified JSONs validation
 *
 * when graphql mutation accepts stringified JSONs as input, we need to validate the JSONs
